<?php
    ...
    }else
    {
        ...
        foreach($result as $res)
        {
            ...
            {
                $response = $response."<tr><td>".$res['ticket_type']."</td><td>".$res['date_submitted']."</td><td><a href='../view/user_profile.php".$res['link']."'>View Source</a></td>";
                
                $response = $response."<td></td><td><button onclick='checkUserAdmin(".$res['user_id'].",10)'>Delete User</button></td><td><button onclick='checkUserAdmin(".$res['ticket_id'].",2)'>Remove this report</button></td></tr>";
                
            } else
            {
                
                $response = $response."<tr><td>".$res['ticket_type']."</td><td>".$res['date_submitted']."</td><td><a href='../view/view_recipe.php".$res['link']."'>View Source</a></td>";
            
                $review_link = explode("review_", $res['link']);

                if(count($review_link)>=2)
                {
                    if($review_link[1]!=0)
                    {
